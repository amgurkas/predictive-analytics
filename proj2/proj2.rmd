...
The data cleaning process is a critical step in preparing our beverage 
manufacturing dataset for pH prediction modeling. Given that pH is a key 
performance indicator (KPI) that must conform to critical ranges, ensuring data 
quality is paramount for accurate predictions. Our data cleaning approach focuses 
...
column_info <- read_excel("data/Copy of Data Columns, Types.xlsx") 
...
if(all(startsWith(names(train_data), "..."))) {
  ...
  train_data <- read_excel("data/StudentData - TO MODEL.xlsx", col_names = FALSE)
...
#### Outlier Detection 
[Insert sentence about identifying outliers using IQR method for numeric values]
...
#### Feature Engineering
[insert sentence about feature engineering.]
...
[insert sentence about data consistency check.]
...
# CHECK IF THIS SHOULD BE DIRECTLY IN THE BODY OF THE .RMD
...
write.csv(train_clean, "data/train_data_cleaned.csv", row.names = FALSE)
write.csv(eval_clean, "data/eval_data_cleaned.csv", row.names = FALSE)

# Save imputation values for future use
saveRDS(imputation_values, "data/imputation_values.rds")
```

### Data Analysis

[insert information on approach here]:

1.  **Exploratory Analysis**: Evaluate the distribution and relationships of the predictors in the training data.
Focusing on:
- Identifying key predictors using a Gradient Boost Model
    - A "good" model will have a low RMSE (Root Mean Squared Error) and high $R^2$. 
      The RMSE measures the average size of the prediction errors—how far the predicted values are       from the actual values.
      $R^2$ (R-squared) measures the proportion of variance in the target (pH) that the model can        explain. A higher $R^2$ suggests the model is uncovering real structure in the data, and is        not just fitting to randomness.

2.  **Identify the Best Modeling Approach**: [insert here]

3.  **Evaluate the Model Performance**: [insert here]

4.  **Predict on Evaluation Data**: [insert here]
...
Based on the Ggpairs plot, it seems that Mnf_Flow and Flow_Pressure_Interaction 
have multicollinearity. If a linear model is used, one of the predictors should 
be removed.

The key predictors are:
1. CO2_Liquid_Ratio (domain knowledge predictor)
2. Saturation_Index (domain knowledge predictor)
3. Alch_Carb_Interaction (domain knowledge predictor)
4. Flow_Pressure_Interaction
5. Temperature
6. Usage cont
7. Alch Rel

```{r data-analysis-modeling-approach, echo=FALSE}
# creating df with only key predictors
train_clean_key_predictors <- train_clean |> 
  select(CO2_Liquid_Ratio,
         Saturation_Index,
         Alch_Carb_Interaction,
...
# running a GBM with key predictors
gbmFit2 <- train(PH ~ ., data = train_clean_key_predictors, 
                 method = "gbm", 
                 trControl = fitControl,
                 ## This last option is actually one
                 ## for gbm() that passes through
...
key_pred <- setdiff(colnames(train_clean_key_predictors), "PH")
...
### !!! If using a linear model, check the VIF values using car::vif() after fitting a linear model.
```

```{r data-analysis-model-perf, echo=FALSE}

```

```{r data-analysis-predict, echo=FALSE}
...
To better understand the direction and shape of the predictor and its effect on 
pH we can visualize it below with scatterplots: 
```{r eda-gbm}
for (var in key_pred) {
